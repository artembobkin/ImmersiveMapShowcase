...
@main
struct BerlinNightDescentApp: App {
    var body: some Scene {
        WindowGroup("Berlin Night Descent") {
            DescentScreen()
        }
...
/// interactive.
private struct DescentScreen: View {
    ...
    private func runAutoRenderIfRequested() async {
        ...
        var shots = BerlinStoryboard.makeShots()
            ...
            while true {
                do {
                    try await videoRecorder.export(shots: shots,
                                                   establish: BerlinStoryboard.overview,
    ...
    private func startPreview() {
        ...
        isTourRunning = true
        showChrome = false
        tour.start(shots: BerlinStoryboard.makeShots(),
                   establish: BerlinStoryboard.overview,
                   loop: true) {
            isTourRunning = false
            showChrome = true
    ...
    private func renderVideo() {
        ...
        panel.nameFieldStringValue = "BerlinNightDescent.mov"
        ...
        Task {
            ...
            do {
                try await videoRecorder.export(shots: BerlinStoryboard.makeShots(),
                                               establish: BerlinStoryboard.overview,
                                               configuration: videoFormat.configuration,
                                               to: url)
                NSWorkspace.shared.activateFileViewerSelecting([url])
            } catch {
                print("Descent video render failed: \(error)")
                NSSound.beep()
            }
        }
    }
